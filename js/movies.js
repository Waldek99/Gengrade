...
async function LoadJson() {
    ...
    ThisIsTheWay(data00.movies);
}


function ThisIsTheWay(movies) {

 	function base00(movies, toggle) {
        ...
        if (toggle == 0) {
            ...
            for (let f = 0, k = movies.length; f < k; f++) {
                layout0A.appendChild(document.createElement("img")).classList.add("img-min00");
                layout0A.children[f].setAttribute("src", movies[f].image_source[0].image_thumb[0]);
                layout0A.children[f].setAttribute("alt", movies[f].image_source[0].image_thumb[0].substring(movies[f].image_source[0].image_thumb[0].lastIndexOf('/')+1));
                layout0A.children[f].id = movies[f].id;
            }
        ...
        if (toggle == 0) {
            ...
            let array_element = movies.pop();
            movies.unshift(array_element);
        }
        for (let s = 0; s < layout02.length; s++) {
            ...
            for (let z = 0, c = 2; z <= c; z++) {
                if (z == 1 ) {
                    ...
                    if (toggle == 1) {
                        layout02[s].children[z].innerHTML = movies[s].title;
                    }
                    else {
                        layout02[s].children[z].innerHTML = movies[s].title;
                    }
        ...
        for (let w = 0; w < layout03.length; w++) {
            ...
            layout03[w].children[0].setAttribute("src", movies[w].image_source[0].image_big[0]);

            layout03[w].children[0].setAttribute("alt", movies[w].image_source[0].image_big[0].substring(movies[w].image_source[0].image_big[0].lastIndexOf('/')+1));
        ...
        for (let b = 0; b < slides00.length; b++) {
            ...
            for (let x = 0, z = movies[b].image_exam[0].image_thumb.length; x < z; x++) {

                layout04[b].children[1].children[0].appendChild(document.createElement("img")).classList.add("img-min00");
                layout04[b].children[1].children[0].children[x].setAttribute("src", movies[b].image_exam[0].image_thumb[x]);
                layout04[b].children[1].children[0].children[x].setAttribute("alt", movies[b].image_exam[0].image_thumb[x].substring(movies[b].image_exam[0].image_thumb[x].lastIndexOf('/')+1));
            }
        }
        ...
        for (let b = 0; b < slides00.length; b++) {
            ...
            for (let x = 0, z = movies[b].image_source[0].image_thumb.length; x < z; x++) {

                layout05[b].children[1].children[0].appendChild(document.createElement("img")).classList.add("img-min00");
                layout05[b].children[1].children[0].children[x].setAttribute("src", movies[b].image_source[0].image_thumb[x]);
                layout05[b].children[1].children[0].children[x].setAttribute("alt", movies[b].image_source[0].image_thumb[x].substring(movies[b].image_source[0].image_thumb[x].lastIndexOf('/')+1));
            }
        }
        ani00(movies);
        selectExamSourc00(movies);
        fullScr00();

 	}
    function ani00(movies) {
            ...
            buttons00[m].addEventListener("click", () => {
                if (buttons00[m].id == "buttonLeft0" + (m-2)) {
                    ...
                    slides00[2].addEventListener("animationend", (event) => {
                        ...
                        let array_element = movies.shift();
                        movies.push(array_element);
                        base00(movies, 1);
                    }, {once: true});
        ...
        divPro00.forEach((element) => {
            ...
            for (let project of element.children) {
                project.addEventListener("click", (event) => {
                    ...
                    let index = movies.findIndex(object => {
    ...
    function selectExamSourc00(movies) {
        ...
        function thirdCondition(iframeSrc, iframeTitle) {
            ...
            document.getElementById("slide01").children[1].children[0].addEventListener("load", (event) => {
                document.getElementById("base-slides").style.height = document.getElementById("slide01").clientHeight + "px";
                if (iframeTitle.substring(iframeTitle.lastIndexOf('.')+1) == "svg" && document.getElementById("title-pro01").innerHTML != "Svg animations 01") {
                    newContent.contentWindow.document.getElementsByTagName("svg")[0].addEventListener("click", (event) => {
                        if (newContent.requestFullscreen) {
                            newContent.requestFullscreen();
                        }
                    });
                   newContent.contentWindow.document.getElementsByTagName("svg")[0].addEventListener("dblclick", (event) => {
                ...
                if (iframeTitle.substring(iframeTitle.lastIndexOf('.')+1) == "svg" && document.getElementById("title-pro01").innerHTML == "Svg animations 01") {
                    newContent.contentWindow.document.getElementsByTagName("svg")[0].setAttribute("height", "100%");
                    newContent.contentWindow.document.getElementsByTagName("svg")[0].setAttribute("width", "100%");
                ...
                example.addEventListener("click", (event) => {
                    for (let a = 0, q = event.target.parentNode.children.length; a <= q; a++) {
                        ...
                        if (event.target == event.target.parentNode.children[a] && event.target.parentNode.classList.contains("examples00")) {
                            if (typeFile[0].some(item => movies[1].image_exam[0].image_big[a].includes(item))) {

                                firstCondition(
                                    movies[1].image_exam[0].image_big[a],
                                    movies[1].image_exam[0].image_big[a].substring(movies[1].image_exam[0].image_big[a].lastIndexOf('/')+1)
                                )
                            }
                            if (typeFile[1].some(item => movies[1].image_exam[0].image_big[a].includes(item))) {

                                secondCondition(movies[1].image_exam[0].image_big[a])
                            }
                            if (typeFile[2].some(item => movies[1].image_exam[0].image_big[a].includes(item))) {
                        ...
                        if (event.target == event.target.parentNode.children[a] && event.target.parentNode.classList.contains("sources00")) {
                            if (typeFile[0].some(item => movies[1].image_source[0].image_big[a].includes(item))) {
                                firstCondition(
                                    movies[1].image_source[0].image_big[a],
                                    movies[1].image_source[0].image_big[a].substring(movies[1].image_source[0].image_big[a].lastIndexOf('/')+1)
                                )
                            }
                            if (typeFile[1].some(item => movies[1].image_source[0].image_big[a].includes(item))) {

                                secondCondition(movies[1].image_source[0].image_big[a])
                            }
                            if (typeFile[2].some(item => movies[1].image_source[0].image_big[a].substring(movies[1].image_source[0].image_big[a].lastIndexOf('/')+1).includes(item))) {
                                console.log(movies[1].image_source[0].image_big[a]);
                                thirdCondition(
                                    movies[1].image_source[0].image_big[a],
                                    movies[1].image_source[0].image_big[a].substring(movies[1].image_source[0].image_big[a].lastIndexOf('/')+1)
                                )
